				     ...
				     struct aw8697_container *aw8697_cont)
{
	...
	aw8697->ram.base_addr =
		(unsigned int)((aw8697_cont->data[0 + shift] << 8) |
			       (aw8697_cont->data[1 + shift]));
	pr_debug(aw8697->dev, "%s: base_addr = %d\n", __func__,
		    aw8697->ram.base_addr);
...
*/
	...
	temp = temp | reg_val;
	pr_debug(aw8697->dev, "%s: almost_empty_threshold = %d\n", __func__,
		    (unsigned short)temp);
	aw8697_i2c_read(aw8697, AW869XX_REG_RTPCFG5, &reg_val);
	temp = temp | (reg_val << 16);
	pr_debug(aw8697->dev, "%s: almost_full_threshold = %d\n", __func__,
		    temp >> 16);
	/* ram */
	shift = aw8697->ram.baseaddr_shift;
				   ...
				   unsigned char play_mode)
{
	...
	case AW8697_HAPTIC_STANDBY_MODE:
		pr_debug(aw8697->dev, "%s: enter standby mode\n", __func__);
	...
	case AW8697_HAPTIC_RAM_MODE:
		pr_debug(aw8697->dev, "%s: enter ram mode\n", __func__);
	...
	case AW8697_HAPTIC_RAM_LOOP_MODE:
		pr_debug(aw8697->dev, "%s: enter ram loop mode\n", __func__);
	...
	case AW8697_HAPTIC_RTP_MODE:
		pr_debug(aw8697->dev, "%s: enter rtp mode\n", __func__);
	...
	case AW8697_HAPTIC_TRIG_MODE:
		pr_debug(aw8697->dev, "%s: enter trig mode\n", __func__);
	...
	case AW8697_HAPTIC_CONT_MODE:
		pr_debug(aw8697->dev, "%s: enter cont mode\n", __func__);
...
static int aw869xx_haptic_get_vbat(struct aw8697 *aw8697)
{
	...
	if (aw8697->vbat > AW8697_VBAT_MAX) {
		...
		pr_debug(aw8697->dev, "%s vbat max limit = %dmV\n", __func__,
			    aw8697->vbat);
	}
	if (aw8697->vbat < AW8697_VBAT_MIN) {
		aw8697->vbat = AW8697_VBAT_MIN;
		pr_debug(aw8697->dev, "%s vbat min limit = %dmV\n", __func__,
			    aw8697->vbat);
	}
	pr_debug(aw8697->dev, "%s aw8697->vbat=%dmV, vbat_code=0x%02X\n",
...
static int aw869xx_haptic_rtp_init(struct aw8697 *aw8697)
{
	...
	while ((!aw869xx_haptic_rtp_get_fifo_afs(aw8697)) &&
	       (aw8697->play_mode == AW8697_HAPTIC_RTP_MODE) &&
	       !atomic_read(&aw8697->exit_in_rtp_loop)) {
		pr_debug(aw8697->dev, "%s rtp cnt = %d\n", __func__,
			    aw8697->rtp_cnt);
		if (!aw8697_rtp) {
			pr_debug(aw8697->dev,
		...
		pr_debug(aw8697->dev, "%s buf_len = %d\n", __func__,
			    buf_len);
		...
		if ((aw8697->rtp_cnt == aw8697_rtp->len) ||
			...
			pr_debug(aw8697->dev, "%s: rtp update complete!\n",
				    __func__);
			aw8697->rtp_cnt = 0;
			//pm_qos_remove_request(&pm_qos_req_vb);
			//mutex_unlock(&aw8697->rtp_lock);
					 ...
					 unsigned char flag)
{
	...
	//pr_debug("%s:aw8697->activate_mode =%d\n", __func__, aw8697->activate_mode);
...
static int aw869xx_rtp_trim_lra_calibration(struct aw8697 *aw8697)
{
	...
	pr_debug(aw8697->dev, "%s microsecond:%ld, theory_time = %d\n",
...
static int aw869xx_rtp_osc_calibration(struct aw8697 *aw8697)
{
	...
	pr_debug(aw8697->dev, "%s: rtp file:[%s], size: %dbytes\n", __func__,
	...
	aw8697->microsecond =
		...
		(aw8697->end.tv_usec - aw8697->start.tv_usec);
	/*calibration osc */
	pr_debug(aw8697->dev, "%s awinic_microsecond: %ld\n", __func__,
		    aw8697->microsecond);
	return 0;
}
...
static int aw869xx_haptic_read_lra_f0(struct aw8697 *aw8697)
{
	...
	if (!f0_reg) {
		...
		aw8697->f0 = (unsigned int)f0_tmp;
		pr_debug(aw8697->dev, "%s lra_f0=%d\n", __func__,
			    aw8697->f0);
	}
...
static int aw869xx_haptic_read_cont_f0(struct aw8697 *aw8697)
{
	...
	if (!f0_reg) {
		...
		f0_tmp = 384000 * 10 / f0_reg;
		aw8697->cont_f0 = (unsigned int)f0_tmp;
		pr_debug(aw8697->dev, "%s cont_f0=%d\n", __func__,
			    aw8697->cont_f0);
	}
	return ret;
}
...
static int aw869xx_haptic_f0_calibration(struct aw8697 *aw8697)
{
	...
	if (aw869xx_haptic_cont_get_f0(aw8697)) {
		...
		pr_debug(aw8697->dev, "%s f0_limit = %d\n", __func__,
			    (int)f0_limit);
		/* calculate cali step */
		f0_cali_step = 100000 *
			       ((int)f0_limit - (int)aw8697->info.f0_pre) /
			       ((int)f0_limit * 24);
		pr_debug(aw8697->dev, "%s f0_cali_step = %d\n", __func__,
			    f0_cali_step);
				       ...
				       unsigned long arg)
{
	...
	int ret = 0;
	pr_debug("%s enter\n", __func__);
	pr_debug(aw8697->dev, "%s: cmd=0x%x, arg=0x%lx\n", __func__, cmd, arg);
				...
				loff_t *offset)
{
	...
	case AW8697_HAPTIC_CMD_READ_REG:
		...
		if (pbuff != NULL) {
			...
			for (i = 0; i < len; i++) {
				pr_debug(aw8697->dev, "%s: pbuff[%d]=0x%02x\n",
					 __func__, i, pbuff[i]);
			}
...
static int aw8697_haptic_init(struct aw8697 *aw8697)
{
	...
	if (aw8697->chip_version == AW8697_CHIP_9X) {
		...
		aw8697->gain = reg_val & 0xFF;
		pr_debug(aw8697->dev, "%s aw8697->gain =0x%02X\n", __func__,
			    aw8697->gain);
...
static void aw869xx_interrupt_setup(struct aw8697 *aw8697)
{
	...
	pr_debug(aw8697->dev, "%s: reg SYSINT=0x%02X\n", __func__, reg_val);
...
static irqreturn_t aw869xx_irq(int irq, void *data)
{
	...
	if (reg_val & AW869XX_BIT_SYSINT_DONEI) {
		pr_debug(aw8697->dev, "%s chip playback done\n", __func__);
	}

	if (reg_val & AW869XX_BIT_SYSINT_FF_AEI) {
		pr_debug(aw8697->dev, "%s: aw869xx rtp fifo almost empty\n",
			    __func__);
		if (aw8697->rtp_init) {
			while ((!aw869xx_haptic_rtp_get_fifo_afs(aw8697)) &&
			       (aw8697->play_mode == AW8697_HAPTIC_RTP_MODE) &&
				  ...
				  struct device_node *np)
{
	...
	for_each_available_child_of_node (np, child_node) {
		...
		if (tmp > HAP_BRAKE_PATTERN_MAX) {
			pr_debug("%s wf-brake-pattern shouldn't be more than %d bytes\n",
				...
				struct device_node *np)
{
	...
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_bemf_config not found\n",
			    __func__);
	...
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_trig_config not found\n",
			    __func__);
	...
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_bst_vol_ram not found\n",
			    __func__);
	val = of_property_read_u32(np, "vib_bst_vol_rtp",
				   &aw8697->info.bst_vol_rtp);
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_bst_vol_rtp not found\n",
			    __func__);

	pr_debug(aw8697->dev, " 20190420_dt aw8697->info.cont_drv_lvl: %d\n",
		    aw8697->info.cont_drv_lvl);
	pr_debug(aw8697->dev,
	...
	for (i = 0; i < 3; i++)
		for (j = 0; j < 5; j++)
			pr_debug(" 20190420_dt aw8697->info.trig_config_9x[%d][%d]: %d\n",
				 ...
				 struct device_node *np)
{
	...
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_bstcfg not found\n", __func__);
	...
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_prctmode not found\n",
			    __func__);
	...
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_sine_array not found\n",
			    __func__);
	...
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_trig_config_9xx not found\n",
			    __func__);
	...
	val = of_property_read_u32(np, "vib_bst_vol_ram_9xx",
				   &aw8697->info.bst_vol_ram);
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_bst_vol_ram_9xx not found\n",
			    __func__);
	val = of_property_read_u32(np, "vib_bst_vol_rtp_9xx",
				   &aw8697->info.bst_vol_rtp);
	if (val != 0)
		pr_debug(aw8697->dev, "%s vib_bst_vol_rtp_9xx not found\n",
			    __func__);

	pr_debug(aw8697->dev, "%s aw8697->info.bst_vol_default_9xx: 0x%x\n",
		    __func__, aw8697->info.bst_vol_default);
	pr_debug(aw8697->dev, "%s aw8697->info.bst_vol_ram_9xx: 0x%x\n",
		    __func__, aw8697->info.bst_vol_ram);
	pr_debug(aw8697->dev, "%s aw8697->info.bst_vol_rtp_9xx: 0x%x\n",
...
static int aw8697_read_chipid(struct aw8697 *aw8697)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		switch (reg) {
		case AW8695_CHIPID:
			pr_debug(aw8697->dev, "%s aw8695 detected\n",
				    __func__);
			...
			return 0;
		case AW8697_CHIPID:
			pr_debug(aw8697->dev, "%s aw8697 detected\n",
				    __func__);
			...
			return 0;
		case AW86905_CHIPID:
			pr_debug(aw8697->dev, "%s aw86905 detected\n",
				    __func__);
			...
			return 0;
		case AW86907_CHIPID:
			pr_debug(aw8697->dev, "%s aw86907 detected\n",
				    __func__);
			...
			return 0;
		case AW86915_CHIPID:
			pr_debug(aw8697->dev, "%s aw86915 detected\n",
				    __func__);
			...
			return 0;
		case AW86917_CHIPID:
			pr_debug(aw8697->dev, "%s aw86917 detected\n",
				    __func__);
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(aw8697->irq_gpio) &&
		...
		pr_debug(&i2c->dev, "%s skipping IRQ registration\n", __func__);
	...
	if (ret < 0) {
		pr_debug(&i2c->dev, "%s error creating sysfs attr files\n",
			 __func__);
		goto err_sysfs;
	}
	...
	ret = create_rb();
	if (ret < 0) {
		pr_debug(&i2c->dev, "%s error creating ringbuffer\n", __func__);
		goto err_rb;
	}
